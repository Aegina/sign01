...
*/
var xDis=0,yDis=0,xMax=0,xMin=0,yMax=0,yMin=0;
var allPoint = [],points = {x:[],y:[]},allTime = [];
(function() {
	function q(b) {
...
//				console.log(this._stroke.x);
//				console.log(this._stroke.y)
				this._lastPoint = b;
				var a = this._stroke,
		...
		this.endStroke = function() {
			...
			if(b) {
				var j = this._stroke,
					...
					g = this.context,
					s = this.changed;
				//  2018 03 14 by zhangqin 修改图片大小
				// 获取所有坐标点
				// (7) x:(7) [265, 277, 285, 352, 386, 502, 527] y:(7) [688, 682, 676, 653, 647, 629, 627]
				function sequence(a,b){return a-b}
         		allPoint.push(j);
//       		eachTime.push(j.t);
				for(var q = 0; q < allPoint.length; q++){
//					allTime = allPoint[q].t.concat(eachTime);
					for(var z = 0; z < allPoint[q].x.length; z++ ){
						points.x.push(allPoint[q].x[z]);
						points.y.push(allPoint[q].y[z]);
					}
				}
				var xArr = points.x.sort(sequence),yArr = points.y.sort(sequence);
				xMax = xArr[xArr.length-1];xMin = xArr[0];yMax = yArr[yArr.length-1];yMin = yArr[0]; // 获取x轴和Y轴的最大最小值
				xDis = xMax-xMin;yDis = yMax-yMin; // 获取绘制区域的宽度和高度
//				// 获取当前点到下一点的时间
//				console.log(allPoint)
//				console.log(allTime)
//				for(var z in allTime){
//					console.log(allTime[z])
//				}
//				function getTP(now,next){
//					var x2 = next.x, x1 = now.x, y2 = next.y, y1 = now.y;
